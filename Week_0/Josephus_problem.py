...
"""
    ...
    1、约瑟夫问题：  
            ...
            输出：出列顺序
    2、约瑟夫问题加强版：
            问题：编号1~N的N个人坐一圈，每人有可以自由输入的密码（不能为1）
        开始一人密码为报数上限，报数到该数出列，并将出列的人的密码作为新的上
        限值
            输出：出列顺序与密码
"""
...
for i in list(range(U)):
    code.append(random.randrange(2,U))
...
def cir(l,i,die):
    if i == N+1:
        ...
        print('他的密码为：',code[die%len(l)-1])

        die = code[die%len(l)-1]
        #print(die, end = '')
        for j in l[0:die-1]:
            l.append(j)
        del l[0:die]

        for k in code[0:die-1]:
            code.append(k)
        del code[0:die]
